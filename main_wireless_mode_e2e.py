...
from model import RL_Policy, Local_IL_Policy, Neural_SLAM_Module, E2E_Policy
...
def main():
    ...
    # Local policy
    l_policy = E2E_Policy(l_observation_space.shape, g_observation_space.shape, envs.action_space.n,
    ...
    global_goals2[0][0] = int(map_goal[0])
    global_goals2[0][1] = int(map_goal[0])
    ...
    for ep_num in range(num_episodes):
        ...
        for step in range(args.max_episode_length):
            ...
            local_wireless = global_input_wireless.to(torch.float32).to(device)
            ...
            #     # April 10 help
            #     if ep_num < 1000 and is_help and going_help:
            ...
            #         else: # 1,2 
            #             b = (power_help_ang + 180) / 360
            #             b = max(b, 10**(-8))
            #             b = min(b, 1-10**(-8))

            #             g_action_true = np.log(b/(1-b))
            ...
            #     writer.add_scalars('actions',{'action_aoa': action_aoa},
            #                                   ep_num*args.max_episode_length+step+1)
            #     print(policy_output_ang,type(policy_output_ang))
            #     writer.add_scalar("actions/policy_output_ang", policy_output_ang, ep_num*args.max_episode_length+step+1)
            #     logging.info(f"LOG: action_aoa = {action_aoa}, policy_output_ang = {policy_output_ang}")

            #     # print(f'Log: Global_Goal Angle {round(global_goals[0][0], 2)}, walk_d {round(global_goals[0][1], 2)}')
            #     print(f'Log: Global_Goal Angle {round(policy_output_ang, 2)}')
            #     # logging.info(f'Log: Global_Goal Angle {round(global_goals[0][0], 2)}, walk_d {round(global_goals[0][1], 2)}')
            #     logging.info(f'Log: Global_Goal Angle {round(policy_output_ang, 2)}')
            #     # converting to local coordinates, first getting the currect origin
            #     start_j = int(start_cj * 100.0/args.map_resolution), int(start_rj * 100.0/args.map_resolution)
            #     o0, o1, _ = origins[0]
            #     o0 = o0/0.05
            ...
            #     temp_reward = Reward_function_april14(
            #         [x_gc_prev, y_gc_prev], # last step agent loc
            #         map_goal, # target loc
            #         prev_blue_point_loc, # last step agent loc
            #         dist, # current step dist bewteen angent and target 
            #         prev_obs, # last step obs
            ...
            #     policy_output_ang_prev = policy_output_ang
            #     x_gc_prev = curr_loc[0]
            #     y_gc_prev = curr_loc[1]
            #     prev_obs = global_input_wireless
            #     # April 14
            #     prev_blue_point_loc = [x_gc, y_gc]
            ...
            #         [temp_reward for env_idx
            #             in range(num_scenes)])).float().to(device)
            #     print(f"LOG: G Reward = {temp_reward}")
            #     logging.info(f"LOG: G Reward = {temp_reward}")
                ...
                for _ in range(args.slam_iterations):
                    ...
                    if args.proj_loss_coeff > 0:
                        proj_loss = F.binary_cross_entropy(b_proj_pred,
                                                           gt_fp_projs)
                        cosb_posests.append(proj_loss.item())
            ...
            if (total_num_steps % args.log_interval == 0) and (not flag_done):
                ...
                if args.train_local and len(l_action_losses) > 0:
                    ...
                    writer.add_scalar("l_action_losses", np.mean(l_action_losses), ep_num*args.max_episode_length+step+1)
