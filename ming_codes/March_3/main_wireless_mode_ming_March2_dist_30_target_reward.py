...
def read_wireless_measure(map_data, curr_cln, curr_row, 
    ...
    Returns
    -------
    wireless_obs : torch tensor with torch.Size([30])
        global obs from wireless (15 old obs, 15 new obs)
    ...
    if prev_obs is None:
        ...
        wireless_obs = torch.from_numpy(np.append(prev_obs, temp)) # torch.Size([30])
    ...
    print(f'Log: Current SNR {round(wireless_obs[10 + 15].item(), 2)}, Angle {round(wireless_obs[0 + 15].item() * 180, 1)}, LS {link_state}')
    return wireless_obs, link_state, temp


def Reward_function_old(previous_dist, dist, previous_l_state, l_state):
    reward = 0
...
def Reward_function_old2(previous_dist, dist, previous_l_state, l_state):
    reward = 0
    ...
    if l_state == 4: #  LOS
        if dist <= 6:
            ...
            logging.info(f'Log: Reach the Target!!!!!!!!!!!!!!!!!')
            reward = 100

    return previous_dist, previous_l_state, reward

def Reward_function_pure(previous_dist, dist, previous_l_state, l_state):
    reward = 0
...
def Reward_function(previous_dist, dist, previous_l_state, l_state):
    ...
    if l_state > previous_l_state:
        reward += 2 * l_state * (1+max(0, previous_dist - dist)) * (l_state - previous_l_state)
    elif l_state < previous_l_state:
        reward += 2 * (5-l_state) * (1-min(0, previous_dist - dist)) * (l_state - previous_l_state)
    else:
        reward += l_state * (- dist)
...
def main():
    ...
    global_input_wireless, link_state, prev_obs = read_wireless_measure(map_data_df,
                                                  start_cj, start_rj, 
                                                  args.map_resolution, 
                                                  indoor_point_idx,
                                                  None)

    assert global_input_wireless.shape == torch.Size([30]), "Wrong wirelss data"
    #
    ...
    """
    Ming
    Let global policy compute the global coordinates
    Map is (480, 480)
    """
    global_goals = [[int(action[0] * 480), int(action[1] * 480)]
    ...
    previous_dist = np.linalg.norm(np.array([start_j[0], start_j[1]]) 
                                    - np.array([map_goal[0], map_goal[1]]))
    previous_l_state = link_state
    print(f"Init start_cj, start_rj {[start_cj, start_rj]}")
    print(f"Init Link State {previous_l_state}")
    print(f"Init Distance {previous_dist}")
    print(f'Init Locs {[start_j[0], start_j[1]]}')
    ...
    for ep_num in range(num_episodes):
        for step in range(args.max_episode_length):
            ...
            # print(f'Log: Target {map_goal[0]} {map_goal[1]}')
            # print(f'Log: Current G-Goal {global_goals[0][1]} {global_goals[0][0]}')
            # print(f"LOG: Wireless Obs {global_input_wireless}")
            assert global_input_wireless.shape == torch.Size([30]), "Wrong wirelss data"
            ...
            if l_step == args.num_local_steps - 1:
                ...
                dist = np.linalg.norm(np.array([curr_loc[0], curr_loc[1]]) 
                                - np.array([map_goal[0], map_goal[1]]))
                # dist = np.linalg.norm(np.array([global_goals[0][1], global_goals[0][0]]) 
                                    #   - np.array([map_goal[0], map_goal[1]])) # first 'G-Goal[0]' is the y, this is not wrong.
                # TODO: Reward function
                previous_dist, previous_l_state, temp_reward = Reward_function_old2(
                        previous_dist, dist, previous_l_state, l_state)
                ...
                print(f'Log: Distance between G-Goal and Target is = {dist}')
                logging.info(f'Log: Distance between G-Goal and Target is = {dist}')
                
                # reward is the negative distance different
                # end Ming

                # g_reward = torch.from_numpy(np.asarray(
                #     [-((global_goals[0][0]-map_goal[0])**2+(global_goals[0][1]-map_goal[1])**2)**0.5 for env_idx
                #      in range(num_scenes)])
                # ).float().to(device)
                ...
                Map is (480, 480)
                """
                global_goals = [[int(action[0] * 480),
                                 int(action[1] * 480)]
                                for action in cpu_actions]
                print("===================================================")
                print(f'Log: Global_Goal {global_goals[0][1]}, {global_goals[0][0]}')
                ...
                print(f'Log: Global transformed {np.array([global_goals[0][1], global_goals[0][0]])}')
