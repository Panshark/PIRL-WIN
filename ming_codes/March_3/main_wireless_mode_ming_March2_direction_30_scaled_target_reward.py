...
def read_wireless_measure(map_data, curr_cln, curr_row, 
    ...
    logging.info(f'LOG: Current Step : {wireless_obs}')
    print(f'Log: Current SNR {round(wireless_obs[10 + 15].item(), 2)}, Angle {round(wireless_obs[0 + 15].item() * 180, 1)}, LS {link_state}')
    logging.info(f'Log: Current SNR {round(wireless_obs[10 + 15].item(), 2)}, Angle {round(wireless_obs[0 + 15].item() * 180, 1)}, LS {link_state}')
    return wireless_obs, link_state, temp

def reward_fun_ming(curr_x, curr_y, target_x, target_y, previous_l_state, l_state, previous_dist):
    """
    Parameters
    ----------
    curr_x : int
    ...
    print(f'Log: current distance {curr_dist}')
    if l_state > previous_l_state:
        ...
        if l_state < 4: # NLOS
            reward = (previous_dist - curr_dist) * l_state
        else: # LOS
            los_factor = (np.exp(-(curr_dist / 240)) - np.exp(-1)) / (1 - np.exp(-1)) # in [0~1]
            reward =  los_factor * ((previous_dist - curr_dist) * l_state)
    ...
    return l_state, curr_dist, reward
...
def main():
    ...
    print(f"Log: Init Distance {previous_dist}")
    ...
    for ep_num in range(num_episodes):
        for step in range(args.max_episode_length):
            ...
            if l_step == args.num_local_steps - 1:
                ...
                global_goals[0][1] = global_goals[0][1] * 20 # walking distance
