...
#     # april9_new
#     minus_power =  min(obs[-2] - prev_obs[-2], 0) # power_diff
#     if angle_diff > 180:
#         angle_diff = 360 - angle_diff
    ...
    for ep_num in range(num_episodes):
        ...
        for step in range(args.max_episode_length):
            ...
            if l_step == args.num_local_steps - 1:
                ...
                temp_reward = Reward_function_march18_april10_new(
                    [x_gc_prev, y_gc_prev],
        ...
        # Save best models
        if ep_num == help_num + args.window_size:
            reward_episode_best = -np.inf
        if reward_episode_best < avg_reward:
            torch.save(g_policy.state_dict(),
                        os.path.join(log_dir, "model_best.global." + str(ep_num)))
        elif flag_done and ep_num > help_num:# Save Done Haozhe
            torch.save(g_policy.state_dict(),
                        os.path.join(log_dir, "model_best.global." + str(ep_num)))
