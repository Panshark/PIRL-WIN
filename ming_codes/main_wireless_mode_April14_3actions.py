...
def Reward_function_april14(
    ...
    delta_dist_agent2goal =  np.linalg.norm(np.array([curr_loc[0], curr_loc[1]]) 
                - np.array([map_goal[0], map_goal[1]]))
    
    # last step blue2goal dist [0, 120]
    delta_dist_blue2goal = np.linalg.norm(np.array([prev_blue_point_loc[0], prev_blue_point_loc[1]]) - np.array([map_goal[0], map_goal[1]]))
...
def main():
    ...
    action_power = global_goals[0][1]
    action_ratio = global_goals[0][2] # 0~1
    policy_output_ang = (action_aoa * action_ratio + action_power * (1 - action_ratio)) * 360 - 180 # (-180 ~ 180) degree


    # logging.info(f"LOG: action_aoa = {action_aoa}, action_power = {action_power}, action_ratio = {action_ratio}, policy_output_ang = {policy_output_ang}")
                ...
                if ep_num < 1000:
                    if random.uniform(-1, 1) >= 0.0:
                        if global_input_wireless[-1].item() == 3 or global_input_wireless[-1].item() == 4:
                            ...
                            c_ratio = 1 - random.uniform(0, 0.1)

                            g_action_true = [np.log(a/(1-a)), np.log(b/(1-b)), np.log(c_ratio/(1-c_ratio))]
                            ...
                            c_ratio = 0 + random.uniform(0, 0.1)
