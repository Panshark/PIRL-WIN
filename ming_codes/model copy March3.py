...
class Wireless_Policy(NNBase):
    def __init__(self, input_shape, recurrent=False, hidden_size=12, downscaling=1, using_extras=False):
        super(Wireless_Policy, self).__init__(recurrent, hidden_size,
                                            hidden_size)


        self.main = nn.Sequential(
            nn.Linear(input_shape[0], 24),
            nn.ReLU(),
        ...
        if using_extras:
            ...
            self.linear1 = nn.Linear(20, hidden_size)
        self.linear2 = nn.Linear(hidden_size, 12)
        self.critic_linear = nn.Linear(12, 1)
        # TODO: Fix extra
        if using_extras:
