...
def main():
    ...
    global_input_wireless = read_wireless_measure(map_data_df,
                                                  start_cj, start_rj, 
                                                  args.map_resolution, 
                                                  indoor_point_idx)
    # print(f"Current locs {start_rj}{start_cj}")
    # print(f"Wireless Obs {global_input_wireless}")
    # print(f'Wireless Obs shape {global_input_wireless.shape}')
    ...
    for ep_num in range(num_episodes):
        for step in range(args.max_episode_length):
            ...
            print(f"LOG: Current locs {start_rj} {start_cj}")
            print(f'Log: Target {map_goal[0]} {map_goal[1]}')
            print(f'Log: Current G-Goal {global_goals[0][0]} {global_goals[0][1]}')
            ...
            start_cj, start_rj, _, _, _, _, _ = planner_pose_inputs[0]
            robot_locs = int((start_cj * 100.0/args.map_resolution + 2) / 3), int((start_rj * 100.0/args.map_resolution + 2) / 3)
            wireless_obs = map_data.loc[(map_data['rxPosInd_1'] == robot_locs[1]) & (map_data['rxPosInd_2'] == robot_locs[0])][columns]
            if wireless_obs.empty:
                global_input_wireless = torch.zeros(wireless_state_shape)
                print("WARNING: The dataframe is empty!!!!!")
            else:
                try:
                    if torch.from_numpy(wireless_obs.to_numpy()).shape[0]==1:
                        global_input_wireless = torch.from_numpy(wireless_obs.to_numpy())
                        global_input_wireless = torch.squeeze(global_input_wireless)
                    elif torch.from_numpy(wireless_obs.to_numpy()).shape[0]==0 or torch.from_numpy(wireless_obs.to_numpy()).shape[0]==g_observation_space:
                        global_input_wireless = torch.from_numpy(wireless_obs.to_numpy())
                        global_input_wireless = torch.squeeze(global_input_wireless)
                    else:
                        global_input_wireless = torch.zeros(global_input_wireless.shape)
                        print("The input data does not exist: ",global_input_wireless)
                except IndexError:
                    print("ERROR: The shape of global_input_wireless is not suitable when generate, shape is: ", torch.from_numpy(wireless_obs.to_numpy()).shape)
            
            """
            
            
            """
            Code for Saving the trajectory
            ...
            if l_step == args.num_local_steps - 1:
                ...
                Reward is the distance bewteen the Target (map_goal) and the global goal
                from the G-Policy
                However, we need to correct the local and gobal coordinate
                """
                # get the currect origin
                o0, o1, _ = origins[0] # num_scenes = 1
                o0 = o0/0.05
                o1 = o1/0.05
                x_truth, y_truth = map_goal
                y_pre = global_goals[0][0] + o1 # first 'G-Goal[0]' is the y, this is not wrong.
                x_pre = global_goals[0][1] + o0
                dist = np.linalg.norm(np.array([x_truth, y_truth]) - np.array([x_pre, y_pre]))
                print("===================================================")
                print(f'Log: Origin {o0}, {o1}')
                print(f'Log: Global_Goal {global_goals[0][1]}, {global_goals[0][0]}')
                print(f'Log: Global transformed {np.array([x_pre, y_pre])}')
                print(f'Log: Truth {np.array([x_truth, y_truth])}')
                print("===================================================")
                print(f'Log: Distance between G-Goal and Target is = {dist}')
                print(f'Log: Reward is = { -dist}')
