...
def Reward_function_new(curr_loc,
                        map_goal, local_dist,
        prev_obs, obs, policy_output_ang, prev_link_state, link_state, max_power, k = 1, gamma=2):
    ...
    # local_dist_reward
    local_dist_reward = max(6-dist, 0)*300

    reward += k*minus_power_reward + dist_reward + gamma*angle_reward + link_punish + local_dist_reward

    logging.info(f"Log: power {minus_power_reward}; dist {dist_reward}; angle {angle_reward}; link_punish {link_punish}, local_dist_reward {local_dist_reward} rewards")
        ...
        for step in range(args.max_episode_length):
            ...
            if l_step == args.num_local_steps - 1:
                ...
                temp_reward = Reward_function_new(
                    [x_gc_prev, y_gc_prev],
                    map_goal, dist, prev_obs, global_input_wireless, policy_output_ang_prev, previous_l_state, l_state, max_snr, k = 1, gamma=2)
