...
def reward_fun_ming(curr_x, curr_y, target_x, target_y, pre_ls, l_state, pre_dist):
    """
    Parameters
    ----------
    curr_x : int
    ...
    if l_state > pre_ls:
        ...
        if l_state < 4: # NLOS
            reward = (pre_dist - curr_dist) * l_state
        else: # LOS
            los_factor = (np.exp(-(curr_dist / 240)) - np.exp(-1)) / (1 - np.exp(-1)) # in [0~1]
            reward =  los_factor * ((pre_dist - curr_dist) * l_state)
