...
def Reward_funciton_new(curr_loc,
                        map_goal,
        obs, policy_output_ang, link_state, max_power, k = 1, gamma=2):
    ...
    minus_power_reward = 0.1 * link_state * minus_power

    # define the dist reward
    dist_reward = 10 * np.exp(-0.2*dist)

    # define the angle reward
    angle_reward = -angle_diff
    ...
    for ep_num in range(num_episodes):
        ...
        for step in range(args.max_episode_length):
            ...
            if l_step == args.num_local_steps - 1:
                ...
                global_goals[0][1] = global_goals[0][1] * 120 # walking distance
                ...
                # Verify Ming
                # """
                # aoa2 = global_input_wireless[16].item()
                # step_d = 12
                ...
                #     start_j,
                #     map_goal,
                #     prev_obs, global_input_wireless[16].item(), l_state, max_snr, k = 1, gamma=2)
                ...
                print(f"LOG: G Reward = {g_reward}")
                logging.info(f"LOG: G Reward = {g_reward}")
                writer.add_scalar("reward", g_reward.item(), ep_num*args.max_episode_length+step+1)
        ...
        writer.add_scalar("reward_episode", reward_sum, ep_num + 1)
        # March 5 Ming 
        logging.info(f"===========================================================================================")
