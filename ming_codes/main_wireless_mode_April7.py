...
def load_wireless_data_goal(map_data_dir, map_goal_dir, map_name, tx_num,
    ...
    print(max_snr)
    assert max_snr == 1, "SNR max should = 1"
...
def read_wireless_measure_simple_input(map_data, curr_cln, curr_row, 
    ...
    print(f'Log: wireless_obs {wireless_obs}')
    logging.info(f'Log: wireless_obs {wireless_obs}')
    print(f'Log: Current SNR {round(wireless_obs[-2].item(), 2)}, Angle {wireless_obs[0].item() * 180}, LS {link_state}')
    return wireless_obs, link_state, temp
...
def Reward_function_April5(curr_loc,
    ...
    minus_power =  (obs[6] - max_power) * 53 # -53 ~ 0
    if angle_diff > 180:
    ...
    for ep_num in range(num_episodes):
        ...
        for step in range(args.max_episode_length):
            ...
            if l_step == args.num_local_steps - 1:
                ...
                #------------------------------------------------------------------------March12
                if not flag_done:
                    print(f'LOG: Reading data step in stop section : {step}')
                    logging.info(f'LOG: Reading data step in stop section : {step}')
                ...
                temp_reward = Reward_function_April5(
                    [x_gc_prev, y_gc_prev],
                ...
                logging.info(f"LOG: Current angle and distance = ang: {policy_output_ang}, x: {x_gc}, y: {y_gc}, true_angle: {global_input_wireless[0] * 180}")
